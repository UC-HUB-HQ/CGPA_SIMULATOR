...
const Calculator = ({handleScore}) => { 
    ...
    const rowElement = (index) => (
        <section key={index} className="grid w-full grid-cols-3 gap-2 mobile:grid-cols-3">
            <div className="flex items-center justify-center col-span-1 text-center mobile:col-span-1 ">
                <input type="text"  placeholder={`GNS 101`} className="pb-2 text-center bg-transparent border-b-2 border-transparent outline-none border-b-black-400 placeholder:text-center placeholder:text-black-400 mobile:w-full placeholder:text-xs" />
            </div>
                ...
                <div>
                    ...
                    <main className="pt-10 ">
                        <section className="grid grid-cols-3 gap-2 w-[95%] mb-10">
                            <div className="col-span-1"><h2 className="text-xl font-semibold text-center mobile:text-sm text-primary-200">Course</h2></div>
                            <div className="col-span-1"><h2 className="text-xl font-semibold text-center mobile:text-sm text-primary-200">Course Units</h2></div>
                            <div className="col-span-1"><h2 className="text-xl font-semibold text-center mobile:text-sm text-primary-200">Grade</h2></div>
                        </section>

                        <section id="calculatorContainer" className="w-full calculatorContainer">
                            {rows?.map( (row, index) => (
                        ...
                        <button className="w-full py-2 font-semibold text-center outline-none bg-primary-500 text-primary-400 rounded-xl" onClick={addRow}>+Add Course</button>
                        <button className="w-full py-2 mt-8 font-semibold text-center text-white outline-none bg-primary-100 rounded-xl" onClick={getGpa}>Get Score</button>
                    </main>
                </div>
                <div id="scoreDisplaySection" className="flex items-center justify-between px-32 py-8 mt-20 bg-white mobile:mt-10 mobile:px-4 rounded-xl mobile:justify-center mobile:gap-20">
                    <h2 className="flex flex-col text-center text-black-200 ">
                        <span className="text-sm ">GPA</span>
                        <span className="text-5xl mobile:text-4xl ">{totalCoursePoint ? (totalCoursePoint  / totalNumberOfUnit).toFixed(2): 0 }</span>
                    </h2>
                    <h2 className="flex flex-col text-center text-primary-600 ">
                        <span className="text-sm">CGPA</span>
                        <span className="text-5xl mobile:text-4xl">{cgpaValue? cgpaValue : 0}</span>
                    </h2>
                </div>
            </div>
        </section>
